...
export const sendOTP = (ws: WebSocketServer) =>
  ...
  ${data["token"]}`;
      ...
      cache.set(reference, _persist, parseInt(body.expiration_time || 5, 10) * 60 * 1000);
...
export const verifyOTP = async (ctx: RouterContext) => {
  try {
    ...
    let token: {[x: string]: string } | undefined = cache.get(body.verification_reference);
    if (!token) {
      ctx.status = 404;
      ctx.body = {
        status: "failed",
